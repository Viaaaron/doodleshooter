...
On the touchscreen, use the left stick to move and the right stick to aim. The right stick has a gentle center response and smooth acceleration, and stops as soon as you lift your thumb. Hold the tall FIRE strip beside it to shoot; extra touch padding and a surrounding buffer prevent missed taps from jumping. Tap open space to jump; swipe down to slide or air dash. Open-screen gestures never turn the camera. Tap AIM to toggle zoom. The curved vertical gun meter matches the right stick's height, with three bullets for the rifle, a shotgun shell, a long sniper cartridge, and a katana. Flick up for the next weapon or down for the previous one, or tap an icon directly. Shake your iPhone to reload; a progress bar appears below the crosshair while the gun reloads. A small RELOAD button is also available.
...
For repeat device builds, run `npm run ios:device`. It checks signing access, recovers a locked or stale keychain through macOS, builds, and opens the game on the connected iPhone/iPad.

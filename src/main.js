...
ctx.hitPlayer = (t, dmg, info) => {
  ...
  if (info.part === 'blade') {
    ...
    if (ret) {
      effects.tracer(info.point, player.eye, INK.RED, 0.03, 0.08); hud.tip('Returned fire', 0.9); input.rumble(0.5, 0.4, 90);
  ...
  if (facing > 0.6 && frontHit && info.source === 'katana' && t.parryWindow) { effects.strokeBurst(info.point, INK.ORANGE, 10, 6, { life: 0.25, size: 0.04 }); audio.shieldHit(t.center); game.hitstop(0.08, 0.15); player.weapons[player.katanaIndex].cooldown = Math.max(player.weapons[player.katanaIndex].cooldown, 0.6); input.rumble(0.6, 0.3, 90); hud.tip('Blocked', 0.9); return; }
...
const MODIFIERS = [
  ...
  { name: 'Caffeine · faster enemies', apply: () => { enemies.mods.speed = 1.35; enemies.mods.damage = 0.85; } },
  { name: 'Heavy ink · harder hits', apply: () => { enemies.mods.speed = 0.9; enemies.mods.damage = 1.4; } },
  { name: 'Swarm · more, weaker enemies', apply: () => { enemies.mods.speed = 1.15; enemies.mods.damage = 0.9; } },
];
const tips = () => [
  `Hold <b>${hud.key('grapple')}</b> to reel in. Tap again to let go.`,
  `Use <b>${hud.key('block')}</b> with the katana to block and return bullets.`,
  'Airborne kills score more. Head for the rooftops.',
  `<b>${hud.key('grenade')}</b>: hold longer to throw farther.`,
  `Press <b>${hud.key('jump')}</b> in the air to double jump.`,
];
...
function startWave(n) {
  ...
  if (boss) { hud.message('WAVE ' + n, enemyName(bossFor(n)) + ' is approaching', 3); audio.bossRoar(player.center); }
  else hud.message('WAVE ' + n, n === 1 ? 'They are coming off the page' : mod.name || choose(['Make your mark', 'Keep drawing', 'Take the high ground', 'Time to slash', 'Send their bullets back']), 2.6);
  audio.wave();
...
function updateWaves(dt) {
  if (game.intermission > 0) {
    game.intermission -= dt; hud.setTimer('Next wave in ' + Math.ceil(game.intermission) + 's');
...
enemies.onKill = (e, info, over) => {
  ...
  if (info.source === 'katana') { label = over ? 'Sliced in two' : 'Slashed'; pts += 50; }
  if (info.source === 'focus') { label = 'Execution'; pts += 150; }
  ...
  if (info.source === 'fall') label = 'Off the page';
  else if (!player.body.onGround && info.source !== 'deflect') { label += ' · Airborne kill'; pts += 40; }
...
function enterFocus() {
  ...
  if (fresh) { audio.focusIn(); hud.tip(`<b>斩击Ready</b> · 按住 ${hud.key('focus')} 冲刺`, 2.2); }
}
...
function onLocalDeath() {
  ...
  hud.kill(kn ? '被 ' + kn + ' 抹除' + (how ? ' · ' + how + (h.crit ? ' Headshot' : '') : '') : 'Erased', 0);
}
function respawnLocal() {
  player.reset(arenaSpawn()); player.name = myName; player.lastHitBy = null; player.lastHit = null; game.state = 'play'; player.shieldT = 2; hud.tip('Spawn shield · 2 seconds', 1.6);
...
function boardHTML(title = 'Free for all') {
  ...
  return `<h3>${title}</h3>${rows.map(([id, s]) => `<div class="${id === net.id ? 'me' : ''}"><span>${s.name}${id === net.id ? ' (you)' : ''}</span><span>${s.kills} kills · ${s.deaths} deaths</span></div>`).join('')}<div class="foot">First to ${FFA_TARGET} kills · Time left ${mmss(matchLeft)} · Lobby ${String(net.aliasCode || net.code || '').replace(/-\d+$/, '')}</div>`;
}
function checkWin() {
...
function endMatch(winner) {
  ...
  hud.setGameplayVisible(false); hud.showScreen(`<h1>${title}</h1><div class="scoreboard">${sortedScores().map(([id, s]) => `<div class="${id === net.id ? 'me' : ''}"><span>${s.name}</span><span>${s.kills} kills · ${s.deaths} deaths</span></div>`).join('')}</div><div class="go" id="overGo">Returning to the lobby…</div>`);
}
...
net.onPeerLeave = (id) => { const nm = (lobby.players.get(id) || {}).name; removeRemote(id); broadcastLobby(); if (inMatch()) { hud.kill((nm || 'Player') + ' Leave了', 0); sendScores(); } };
net.onDisconnect = () => { if (lobby.order && lobby.order.some((id) => id !== lobby.hostId)) migrateHost(); else leaveOnline('房主Leave了Lobby'); };
...
async function _migrateHost() {
  ...
  const roster = (lobby.order || []).filter((id) => id !== oldHost && lobby.players.has(id)); if (!roster.length || !base) { leaveOnline('房主Leave了Lobby'); return; }
  ...
  hud.message('房主Leave了', successor === myId ? '现在由你担任房主' : '正在迁往新房主…', 2.6);
  if (successor === myId) {
    let ok = false;
    ...
    net.onAlias = () => { broadcastLobby(); hud.kill('LobbyCode ' + base + ' 已恢复', 0); }; net.claimAlias(base);
    ...
    if (!joined) { leaveOnline('房主Leave，对局已丢失'); return; }
...
net.onPeerJoin = (from, meta) => {
  ...
  if (game.state === 'play' || game.state === 'dying') { if (!scores.has(from)) scores.set(from, { name, kills: 0, deaths: 0 }); net.sendTo(from, 'start', { late: true, spawn: farthestSpawnIndex(), map: lobby.map || mapKey, broken: level.breakables.filter((b) => !b.alive).map((b) => b.id) }); sendScores(); hud.kill(name + ' Join了', 0); }
};
net.on('lobby', (d) => {
...
net.on('leave', (d) => { const nm = (lobby.players.get(d.id) || {}).name; removeRemote(d.id); if (inMatch()) hud.kill((nm || 'Player') + ' Leave了', 0); renderLobby(); });
...
net.on('pdead', (d, from) => {
  const r = remote.get(from); const vn = r ? r.name : 'Player'; const kn = d.killer && scores.get(d.killer) ? scores.get(d.killer).name : null;
  ...
  else hud.kill(kn ? kn + ' 抹除了 ' + vn + how : vn + ' Off the page', 0);
...
net.on('fell', (d, from) => { if (!net.isHost) return; const sc = scores.get(from); if (sc) { sc.kills = Math.max(0, sc.kills - 1); sendScores(); net.send('feed', { text: sc.name + ' Off the page · -1' }); hud.kill(sc.name + ' Off the page · -1', 0); } });
net.on('feed', (d) => hud.kill(String(d.text || ''), 0));
player.onFall = () => {
  if (!online() || !inMatch()) return;
  hud.kill('Off the page · 击杀 -1', 0);
  if (net.isHost) { const sc = scores.get(net.id); if (sc) { sc.kills = Math.max(0, sc.kills - 1); sendScores(); net.send('feed', { text: sc.name + ' Off the page · -1' }); } }
  else net.send('fell', {});
};
...
function idleUpdate(dt) {
  ...
  if (idle > limit && canDrop) { const back = net.isHost ? null : String(net.aliasCode || net.code || '').replace(/-\d+$/, ''); leaveOnline(net.isHost ? 'Lobby已关闭：全员挂机' : '因挂机被移出'); lobby.rejoinCode = back; if (back) showStart(); return; }
...
function netUpdate(dt) {
  ...
  if (inMatch() && !game.over) { if (clockOn) matchLeft = Math.max(0, matchLeft - dt); if (net.isHost) { clockT -= dt; if (clockT <= 0) { clockT = 2; net.send('clock', { left: Math.round(matchLeft), on: clockOn }); } } hud.setTimer(clockOn ? mmss(matchLeft) : '有玩家Join后开始计时'); }
...
async function quickPlay() {
  ...
  setStatus('暂无开放的Lobby · 正在为你开一个Public lobbies…');
  await createLobby(true);
}
function friendlyError(err) {
  const m = String(err && err.message || err || ''); if (!m) return '出了点问题';
  if (/networking library/.test(m)) return '无法加载联机库 · 请检查网络并Refresh页面';
  if (/timed out|signalling/.test(m)) return '无法连接匹配服务器 · 请检查你的网络';
  if (/no lobby with that code/.test(m)) return '找不到该Code对应的Lobby · 和朋友核对一下Code';
  if (/no answer/.test(m)) return '找到了Lobby但无法连接 · 你们中可能有人处于阻止直连的网络';
  if (/full/.test(m)) return '该LobbyFull · 试试其他Code';
  if (/leave the lobby/.test(m)) return '请先Leave你当前的Lobby';
  return m;
}
...
function mainHTML() {
  return `<h1>Doodle District</h1><h2>Survive the page</h2>
    <div class="mainbtns"><button type="button" class="start" id="soloBtn">PLAY SOLO<i>Survive the waves</i></button><button type="button" id="onlineBtn">PLAY ONLINE<i>Free for all · up to 10 players</i></button></div>
    ${mapHTML(mapKey, true)}${controlsHTML()}${settingsHTML()}${checkpointHTML()}${best ? `<div class="beststat">Best: ${best}</div>` : ''}`;
}
function onlineHTML() {
  return `<h1>PLAY ONLINE</h1><h2>Free for all · First to ${FFA_TARGET} kills · up to 10 players</h2>
    <div class="online" id="online">
      ...
      <div class="row"><button type="button" class="big" id="quickBtn">Quick play</button><span class="hint">Join an open lobby, or create one if none is available</span></div>
...
function lobbyHTML() {
  ...
  return `<h1>Lobby</h1><h2>Free for all · First to ${FFA_TARGET} kills · ${n}/${net.maxPlayers} players</h2>
    <div class="online" id="online">
      ...
      <div class="hint">${lobby.isPublic ? 'Public lobby: anyone can join with Quick play or this code' : 'Private lobby: friends can enter this code under Play online'}</div>
      ...
      <div class="status" id="status">${esc(lobby.status || '')}</div><div class="hint">Anyone can start · ${n < 2 ? 'Players can join after the match starts' : n + ' players ready'}</div>
    </div>`;
}
let lobbyList = null, listBusy = false;
function lobbyListHTML() {
  if (listBusy) return '<div class="hint">Looking for lobbies…</div>';
  if (!lobbyList) return '<div class="hint">Tap Refresh to find open lobbies</div>';
  if (!lobbyList.length) return '<div class="hint">Tap Quick play to join a lobby</div>';
  return lobbyList.map((l) => `<div class="lobbyrow"><span class="code">${esc(l.code)}</span><span>${esc(l.hostName || 'Player')}’s lobby</span><span>${l.players}/${l.max}${l.inMatch ? ' · playing' : ''}</span>${l.full ? '<span class="status">Full</span>' : `<button type="button" data-join="${esc(l.code)}">Join</button>`}</div>`).join('');
}
...
function wireOnline() {
  ...
  if (q('joinBtn')) { q('joinBtn').addEventListener('click', () => { const c = q('codeBox').value.trim().toUpperCase(); if (!c) { setStatus('Enter your friend’s lobby code'); return; } lockButtons(box); joinLobby(c); }); q('codeBox').addEventListener('keydown', (e) => { if (e.key === 'Enter') q('joinBtn').click(); }); }
...
function showPause() {
  if (online()) {
    hud.showScreen(`<h1>Menu</h1><h2>Free for all · Lobby ${String(net.aliasCode || net.code || '').replace(/-\d+$/, '')}</h2><div class="scoreboard">${sortedScores().map(([id, s]) => `<div class="${id === net.id ? 'me' : ''}"><span>${esc(s.name)}</span><span>${s.kills} kills · ${s.deaths} deaths</span></div>`).join('')}</div>${controlsHTML()}${settingsHTML()}<div class="online" id="online"><div class="row"><button type="button" class="alt" id="leaveBtn">Leave match</button></div></div><div class="go">Tap to resume</div>`);
    wireSettings(); wireOnline(); return;
  }
  hud.showScreen(`<h1>Paused</h1><h2>Wave ${game.wave} · Score ${game.score}</h2>${controlsHTML()}${settingsHTML()}${menuBtnHTML()}<div class="go">Tap to resume</div>`);
  wireSettings(); wireMenuBtn();
}
function showClickToPlay() { hud.showScreen(`<h1>Match started</h1><h2>Free for all · First to ${FFA_TARGET} kills</h2><div class="go">Tap to play</div>`); }
function showDead() {
  hud.setGameplayVisible(false); const nb = game.score > best; if (nb) { best = game.score; localStorage.setItem('doodle_best', String(best)); }
  hud.showScreen(`<h1>Erased</h1><div class="stats">Survived <b>${game.wave}</b> waves · <b>${game.kills}</b> kills · Score <b>${game.score}</b>${nb ? ' · <b>New best</b>' : ` · Best ${best}`}</div>${checkpointHTML()}${menuBtnHTML()}<div class="go">Tap to play again</div>`);
...
function startMatch(late, spawnIdx) {
  ...
  refreshScoreHud(); hud.message('Free for all', late ? 'You joined a match in progress' : '先到 ' + FFA_TARGET + ' kills · ' + Math.round(FFA_TIME / 60) + ' 分钟 · 人人都是目标', 3);
  ...
  setTimeout(() => { if (game.state === 'play' && !input.pointerLocked && !input.usingGamepad && !input.touchEnabled) { game.menu = true; showClickToPlay(); } }, 250);
}
function pause() { if ((game.state !== 'play' && !(game.state === 'dying' && online())) || game.menu) return; if (!online()) game.state = 'pause'; game.menu = true; input.touch?.setActive(false); showPause(); audio.reelLoop(false); }
...
Object.assign(window.__game, { startWave, updateWaves, begin, beginAtWave, jumpToWave, resetGame, pause, resume, spawnPickup, focusCandidate, enterFocus, pickSpawn, startMatch, createLobby, joinLobby, quickPlay, leaveOnline, hostStart });
hud.onScreenClick = () => {
  const st = game.state;
...
hud._touch = input.touchEnabled; hud.setDevice(input.usingGamepad); applySettings(); hud.setWeapon(player.weapon.name, player.weapon.hint); showStart();
...
function step(now) {
  ...
  if (st === 'play' && !game.menu && !input.pointerLocked && !input.usingGamepad && !input.touchEnabled) { lockTipT -= dt; if (lockTipT <= 0) { lockTipT = 2.5; hud.tip('Click to capture the mouse', 2); } }
  let scale = 1;
  ...
  if (playing) {
    ...
    if (st === 'dying') {
      ...
      if (online()) {
        ...
        if (left > 0) { if (left !== before || game.deathT <= dt) hud.message(String(left), 'Respawning soon', 1.1); }
          ...
          game.promptT -= dt; if (game.promptT <= 0) { game.promptT = 1.4; hud.message('Ready', `Tap any control to respawn`, 1.5); }
